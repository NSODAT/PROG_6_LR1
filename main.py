...
def main():
    ...
    total_quantity = get_total_quantity(data, 22121)

    # Выводим результат
    print(f"Общее количество товара с кодом 22121: {total_quantity}")
...
def get_total_quantity(data: List[dict], stock_code: int) -> int:
    ...
    #        result =
    result = sum(
        [_el['Quantity'] for _el in data if _el['StockCode'] == stock_code])
    return result
